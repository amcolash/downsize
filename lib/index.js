...
exports.image = function (source, target, options, callback) {
  ...
  image.orientation((err, orientation) => {
    // console.log(source, orientation)

    switch(orientation) {
      case "TopLeft": // 1 = Horizontal(normal)
      default:
        break;
      case "TopRight": // 2 = Mirror horizontal
        image.flop()
        break;
      case "BottomRight": // 3 = Rotate 180
        image.rotate('white', 180)
        break;
      case "BottomLeft": // 4 = Mirror vertical
        image.flip()
        break;
      case "LeftTop": // 5 = Mirror horizontal and rotate 270 CW
        image.flop()
        image.rotate('white', 270)
        break;
      case "RightTop": // 6 = Rotate 90 CW
        image.rotate('white', 90)
        break;
      case "RightBottom": // 7 = Mirror horizontal and rotate 90 CW
        image.flop()
        image.rotate('white', 90)
        break;
      case "LeftBottom": // 8 = Rotate 270 CW
        image.rotate('white', 270)
        break;
    }
    ...
    image.write(target, () => {
      ep.open()
      .then(() => {
        ep.writeMetadata(target, {
          orientation: ''
        }, ['overwrite_original']).then(callback())
      })
    })
  })
}
